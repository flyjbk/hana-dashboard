...
document.getElementById('file-input').addEventListener('change', function(e) {
  ...
  chip.innerHTML = '📎 ' + file.name + ' <span onclick="clearAttach()">×</span> (업로드 중...)';
  var reader = new FileReader();
  reader.onload = async function(ev) {
    var base64 = ev.target.result.split(',')[1];
    try {
      var res = await fetch(LOG_BASE + '/upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'ngrok-skip-browser-warning': 'true' },
        body: JSON.stringify({ filename: file.name, data: base64, size: file.size })
      });
      var data = await res.json();
      if (data.error) {
        chip.innerHTML = '⚠️ ' + data.error + ' <span onclick="clearAttach()">×</span>';
        return;
      }
      _attachText = data.text;
      _attachName = data.filename;
      var info = data.truncated ? ' (앞 8000자)' : ' (' + data.chars + '자)';
      chip.innerHTML = '📎 ' + file.name + info + ' <span onclick="clearAttach()">×</span>';
    } catch(err) {
...
async function sendMessage() {
  ...
  if (!text) return;
  input.value = '';
  appendMsg('user', _attachName ? '[📎 ' + _attachName + '] ' + text : text);
  var msgContent = _attachText
    ? '[첨부파일: ' + _attachName + ']\n\n' + _attachText + '\n\n---\n' + text
    : text;
